...
function getStatistic(employeeDataLinked){
    let employeeData =  employeeDataLinked;
    let totalAge = 0;
    let ages = [];
    let workloads = [];
    let totalFemaleWorkload = 0;
    let femaleCount = 0;
    let workloadCounts = {10: 0, 20: 0, 30: 0, 40: 0};


    for (let i = 0; i < employeeData.length; i++) {
        let employee = employeeData[i];
        let age = calculateAge(employee.birthdate);
        totalAge += age;
        ages.push(age);
        workloads.push(employee.workload);

        workloadCounts[employee.workload] = workloadCounts[employee.workload] + 1;

        if (employee.gender === "female") {
            totalFemaleWorkload += employee.workload;
            femaleCount++;
        }
    }

    ages.sort((a, b) => a - b);
    let averageAge = totalAge / employeeData.length;
    let medianAge = ages.length % 2 === 0 ?
        (ages[Math.floor(ages.length / 2) - 1] + ages[Math.floor(ages.length / 2)]) / 2 :
        ages[Math.floor(ages.length / 2)];

    workloads.sort((a, b) => a - b);
    let medianWorkload = workloads.length % 2 === 0 ?
        (workloads[Math.floor(workloads.length / 2) - 1] + workloads[Math.floor(workloads.length / 2)]) / 2 :
        workloads[Math.floor(workloads.length / 2)];
    let averageFemaleWorkload = femaleCount > 0 ? totalFemaleWorkload / femaleCount : 0;

    employeeData.sort((a, b) => a.workload - b.workload);
    ...
    const dtoOut = {
        ...
        averageWomenWorkload: Math.floor(averageFemaleWorkload),
...
function main() {
    console.log(getStatistic(generateEmployeeData()));
}

main(
    getStatistic(generateEmployeeData(dtoIn))
);
